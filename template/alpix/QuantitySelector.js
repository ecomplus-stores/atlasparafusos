...
export default {
  ...
  data () {
    return {
      ...
      openDropdownId: null,
      customQnts: this.items.reduce((acc, item) => {
        acc[item._id] = null
        return acc
      }, {})
    }
  ...
  methods: {
    ...
    selectPreset (item, n) {
      if (n !== this.selectedQnts[item._id]) {
        this.changeQnt(item, n - this.selectedQnts[item._id])
      }
      this.openDropdownId = null
    },

    applyCustomQnt (item) {
      const val = parseInt(this.customQnts[item._id], 10)
      if (val > 0 && val !== this.selectedQnts[item._id]) {
        this.changeQnt(item, val - this.selectedQnts[item._id])
      }
      this.openDropdownId = null
    }
  },
