...
export default {
  ...
  computed: {
    ...
    progressiveDiscount () {
      // Pega os flags do produto ou da variação selecionada
      const flags = this.selectedVariationId 
        ? (this.selectedVariation.flags || this.body.flags)
        : this.body.flags
      
      if (!flags || !this.qntToBuy) return 0
      const discount = getProgressiveDiscount(flags, this.qntToBuy)
      if (!discount) return 0
      
      // Para cálculos internos, retorna a porcentagem ou converte para porcentagem equivalente
      if (discount.type === '%') {
        return discount.value
      } else if (discount.type === '$') {
        // Se é desconto em valor, tenta calcular equivalente em percentualif (getPrice(this.body) > 0) {
          return Math.round((discount.value * 100) / getPrice(this.body))
    ...
    progressiveDiscountData () {
      // Retorna os dados brutos do desconto progressivo para uso no template
      const flags = this.selectedVariationId 
        ? (this.selectedVariation.flags || this.body.flags)
        : this.body.flags
      
      if (!flags || !this.qntToBuy) return null
      return getProgressiveDiscount(flags, this.qntToBuy)
    }
  },
  ...
  methods: {
    ...
    getParsedFlags (flags) {
      if (!flags || !Array.isArray(flags)) return []
      const basePrice = (this.selectedVariationId && this.selectedVariation.price) || this.body.price || 0
      return flags.map(flag => {
        const [quantity, type, value] = flag.split('|')
        const floatValue = parseFloat(value)
        let price = basePrice
        if (type === '%') {
          price = basePrice * (1 - floatValue / 100)
        } else if (type === '$') {
          price = Math.max(0, basePrice - floatValue)
        }
        return {
          ...
          display: `A partir de ${quantity} unidades: ${type === '%' ? value + '% de desconto' : 'R$ ' + floatValue.toFixed(2) + ' de desconto'}`
        }
      })
    },
    ...
    buy () {
      ...
      // DEPOIS
      if (this.canAddToCart) {
        const productToCart = { ...product, customizations }
        const discount = this.progressiveDiscountData
        if (discount) {
          const basePrice = (variationId && this.selectedVariation.price) || product.price
          if (basePrice) {
        ...
        console.log(`apx`,productToCart)
        ecomCart.addProduct(productToCart, variationId, this.qntToBuy)
      }
      this.isOnCart = true
    },
