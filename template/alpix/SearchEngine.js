    ...
    isTableLayout () {
      if (typeof window === 'object' && window.storefront && window.storefront.context) {
        ...
        if (context.resource === 'categories' && context.body && context.body.slug) {
          ...
          const tableCategories = window.productCardTableCategories || []
          const tableLayoutExclude = window.productCardTableLayoutExclude || false
          const isInList = tableCategories.includes(categorySlug)
          return tableLayoutExclude ? !isInList : isInList
        }
      }
      return false
    }
